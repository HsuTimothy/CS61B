...
abstract class Board extends Observable {
   ...
   /** Return true iff row R and column C denotes a valid square. */
    ...
    /** Return true iff someone has won the game. */
    abstract boolean winYet();
    ...
    public String toString() {
        ...
        for (int x = 1; x < size() + 1; x++) {
            ...
            for (int y = 1; y < size() + 1; y++) {
                fancy.append(get(x, y).getSpots());
                if (get(x, y).getSide() == RED) {
                    fancy.append("r");
                } else if (get(x, y).getSide() == BLUE) {
                    fancy.append("b");
                } else {
                    fancy.append("-");
                }
